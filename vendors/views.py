...
def create_and_list_vendor(request):
    if request.method == 'GET':
        request_object = Vendor.objects.filter(is_deleted=False)
        serializer = VendorSerializer(request_object, many=True)
...
def list_update_delete_vendor_by_id(request, pk):
    try:
        request_object = Vendor.objects.get(pk=pk)
    except Vendor.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)
    if request.method == 'GET':
        try:
            request_object = Vendor.objects.filter(
